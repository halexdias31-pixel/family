...
/* ==================================================================================================
 * PRACTICE PAPERS, WRITTEN HERE, WITH THEIR ANSWERS.
 *
 * `seedPastPapers` above puts 78 Edexcel GCSE papers in the library and every one of them is a LINK
 * to Pearson's own PDF. That is the right shape for those: linking to somebody's paper is linking,
 * and the paper opens on their site with their name on it.
 *
 * WHAT IT CANNOT DO IS FILL THE QUESTIONS TAB. A past paper's questions are Pearson's copyright, and
 * typing them into this business's own database — where the site serves them, the basket prints
 * them, and the print is charged for — is republishing somebody else's exam paper and selling it.
 * The question cards, the answer boxes and the print queue have all been built against an empty
 * tab, so the machinery is there and there is nothing in it.
 *
 * SO THE QUESTIONS HERE ARE OURS. Written to the same specification — Higher tier, non-calculator,
 * 80 marks over twenty questions — and every one of them original, so the paper can be printed,
 * sold, marked and put on a wall without asking anybody. A practice paper is also the thing a tutor
 * actually wants between past papers: the real ones are finite and a student who has done them all
 * has nothing left to sit.
 *
 * NAMED AS WHAT IT IS. "Practice Paper 1" and `exam_board: '@family.'`, because a student who
 * revises from something believing it came from Edexcel and then meets Edexcel's phrasing in the
 * hall has been misled by the label rather than taught by the paper.
 *
 * ADDING THE NEXT ONE IS ADDING AN ARRAY. `PRACTICE` is a list of papers and a paper is a list of
 * questions; nothing below this line knows how many of either there are.
 *
 * IDEMPOTENT TWICE OVER. The resource row is matched on its name, and every question row carries a
 * `row_id` built from the paper and the question — `PP1-Q7b` — so a second run finds them all and
 * writes nothing. That matters more here than on the paper list: a question somebody has corrected
 * in the sheet must survive the seeder being run again.
 * ================================================================================================ */

/* A question is: number, part, marks, topic, the question, the answer.
   `part` IS BLANK ON A SINGLE-PART QUESTION and `paperBody_` draws no `(a)` for it, which is what
   an exam paper does — Q8 is not "Q8 (a)" when there is no (b). */
const PRACTICE = [{
  name: 'Practice Paper 1 (Non-calculator)',
  tier: 'Higher',
  keyStage: 'KS4',
  subject: 'Maths',
  year: 2026,
  /* 80 MARKS, AND IT IS ADDED UP FROM THE ROWS RATHER THAN WRITTEN DOWN — `paperBody_` sums them,
     so a total typed here would be a second copy free to disagree with the paper. It is 80; if a
     question is edited it will be whatever it then is, and the card will say so. */
  qs: [
  ['1',  '',  4, 'Number',   'Write <em>(3 + &radic;5)(2 &minus; &radic;5)</em> in the form <em>a + b&radic;5</em>, where <em>a</em> and <em>b</em> are integers.',
                             '1 &minus; &radic;5, so a = 1 and b = &minus;1. Expanding: 6 &minus; 3&radic;5 + 2&radic;5 &minus; 5.'],
  ['2',  '',  3, 'Number',   'Work out <em>16<sup>3/4</sup> &times; 2<sup>&minus;2</sup></em>.',
                             '2. 16<sup>3/4</sup> = 8 and 2<sup>&minus;2</sup> = 1/4, so 8 &times; 1/4 = 2.'],
  ['3',  '',  4, 'Number',   'Work out <em>(4.8 &times; 10<sup>5</sup>) &divide; (1.2 &times; 10<sup>&minus;2</sup>)</em>. Give your answer in standard form.',
                             '4 &times; 10<sup>7</sup>. 4.8 &divide; 1.2 = 4, and 10<sup>5 &minus; (&minus;2)</sup> = 10<sup>7</sup>.'],
  ['4',  '',  3, 'Number',   'In a sale, a coat is reduced by 30%. The sale price is &pound;84. Work out the price before the sale.',
                             '&pound;120. The sale price is 70% of the original, so 84 &divide; 0.7 = 120.'],
  ['5',  '',  4, 'Algebra',  'Solve the simultaneous equations <em>3x + 2y = 19</em> and <em>x &minus; 2y = &minus;3</em>.',
                             'x = 4, y = 3.5. Adding the equations gives 4x = 16, then 4 &minus; 2y = &minus;3.'],
  ['6',  'a', 2, 'Algebra',  'Find an expression for the <em>n</em>th term.',
                             '6n &minus; 1. The terms go up by 6 each time, and 6 &times; 1 &minus; 1 = 5.'],
  ['6',  'b', 2, 'Algebra',  'Ali says that 200 is a term in this sequence. Is he right? You must show your working.',
                             'No. 6n &minus; 1 = 200 gives n = 33.5, which is not a whole number, so 200 is not a term.'],
  ['7',  '',  4, 'Algebra',  'Expand and simplify <em>(2x &minus; 3)(x + 4)(x &minus; 1)</em>.',
                             '2x<sup>3</sup> + 3x<sup>2</sup> &minus; 17x + 12. First (2x &minus; 3)(x + 4) = 2x<sup>2</sup> + 5x &minus; 12, then multiply by (x &minus; 1).'],
  ['8',  '',  3, 'Algebra',  'Solve <em>x<sup>2</sup> &minus; 5x &minus; 14 = 0</em>.',
                             'x = 7 or x = &minus;2. Factorising gives (x &minus; 7)(x + 2) = 0.'],
  ['9',  '',  4, 'Algebra',  'Solve the inequality <em>2x<sup>2</sup> + 5x &minus; 3 &lt; 0</em>.',
                             '&minus;3 &lt; x &lt; 1/2. Factorising gives (2x &minus; 1)(x + 3) &lt; 0, which is negative between its two roots.'],
  ['10', 'a', 2, 'Probability', 'Work out the probability that both counters are red.',
                             '5/14. (5/8) &times; (4/7) = 20/56.'],
  ['10', 'b', 2, 'Probability', 'Work out the probability that the two counters are different colours.',
                             '15/28. Red then blue or blue then red: 2 &times; (5/8) &times; (3/7) = 30/56.'],
  ['11', '',  4, 'Number',   'A rectangle measures 8.4 cm by 5.2 cm. Each length is given to 1 decimal place. Work out the upper bound for the area of the rectangle.',
                             '44.3625 cm<sup>2</sup>. The upper bounds are 8.45 cm and 5.25 cm.'],
  ['12', '',  4, 'Geometry', 'Two solids are mathematically similar. Their surface areas are 45 cm<sup>2</sup> and 80 cm<sup>2</sup>. The volume of the smaller solid is 27 cm<sup>3</sup>. Work out the volume of the larger solid.',
                             '64 cm<sup>3</sup>. The area ratio 45:80 is 9:16, so the length ratio is 3:4 and the volume ratio is 27:64.'],
  ['13', '',  5, 'Geometry', 'A, B and C are points on a circle with centre O. Angle ABC = 58&deg;. Work out the obtuse angle AOC. Give a reason for your answer.',
                             '116&deg;. The angle at the centre is twice the angle at the circumference when both stand on the same arc.'],
  ['14', '',  3, 'Geometry', 'OA = <strong>a</strong> and OB = <strong>b</strong>. M is the midpoint of AB. Express OM in terms of <strong>a</strong> and <strong>b</strong>.',
                             '&frac12;(<strong>a</strong> + <strong>b</strong>). OM = <strong>a</strong> + &frac12;(<strong>b</strong> &minus; <strong>a</strong>).'],
  ['15', '',  4, 'Algebra',  'Simplify fully <em>(x<sup>2</sup> &minus; 9) &divide; (x<sup>2</sup> + x &minus; 12)</em>.',
                             '(x + 3)/(x + 4). The top is (x &minus; 3)(x + 3) and the bottom is (x + 4)(x &minus; 3).'],
  ['16', 'a', 2, 'Algebra',  'Work out <em>fg(4)</em>.',
                             '46. g(4) = 16, then f(16) = 3 &times; 16 &minus; 2.'],
  ['16', 'b', 2, 'Algebra',  'Find <em>f<sup>&minus;1</sup>(x)</em>.',
                             '(x + 2)/3. Rearranging y = 3x &minus; 2 gives x = (y + 2)/3.'],
  ['17', '',  4, 'Algebra',  'Write <em>x<sup>2</sup> + 8x + 3</em> in the form <em>(x + a)<sup>2</sup> + b</em>, and hence write down the coordinates of the turning point of the curve <em>y = x<sup>2</sup> + 8x + 3</em>.',
                             '(x + 4)<sup>2</sup> &minus; 13, so the turning point is (&minus;4, &minus;13).'],
  ['18', '',  4, 'Algebra',  'Prove that the sum of any three consecutive integers is a multiple of 3.',
                             'n + (n + 1) + (n + 2) = 3n + 3 = 3(n + 1), which is 3 times an integer.'],
  ['19', '',  4, 'Ratio',    'A box holds red and blue counters in the ratio 3:5. 12 more red counters are put in. The ratio is now 3:4. How many blue counters are in the box?',
                             '80. With red = 3k and blue = 5k, (3k + 12)/(5k) = 3/4 gives k = 16, so blue = 80.'],
  ['20', 'a', 2, 'Graphs',   'Write down the coordinates of the minimum point of <em>y = f(x) + 3</em>.',
                             '(2, &minus;2). Adding 3 moves the curve up 3.'],
  ['20', 'b', 2, 'Graphs',   'Write down the coordinates of the minimum point of <em>y = f(x &minus; 1)</em>.',
                             '(3, &minus;5). Subtracting 1 inside the bracket moves the curve right 1.'],
  ['20', 'c', 3, 'Graphs',   'Describe fully what happens to the minimum point on <em>y = &minus;f(x)</em>.',
                             'It becomes a MAXIMUM at (2, 5). The curve is reflected in the x-axis.'],
  ],
  /* THE SHARED WORDING ABOVE A MULTI-PART QUESTION — one row each, `kind: 'stem'`, printed once
     above its parts by `paperBody_`. Written apart from the parts because that is what it is: a
     stem belongs to the question, not to (a). */
  stems: [
  ['6',  'Here are the first four terms of an arithmetic sequence: <strong>5&nbsp;&nbsp;11&nbsp;&nbsp;17&nbsp;&nbsp;23</strong>'],
  ['10', 'A bag holds 5 red counters and 3 blue counters. Two counters are taken at random, without replacement.'],
  ['16', '<em>f(x) = 3x &minus; 2</em> and <em>g(x) = x<sup>2</sup></em>.'],
  ['20', 'The graph of <em>y = f(x)</em> has a single minimum point at (2, &minus;5).'],
  ],
}];

/**
 * WRITE THE PRACTICE PAPERS AND THEIR QUESTIONS.
 *
 * ONE TAB, AND IT IS NOT THE ONE I FIRST WROTE THIS AGAINST. The obvious shape is a row on
 * `resources` for the paper and question rows pointing at its `resource_id`, the way every other
 * library item works. `allTopics` does not read `resources` at all: a paper IS its questions, the
 * id is whatever `paper_id` says, and the paper-level facts — name, subject, tier, board, key
 * stage, year — are columns on the QUESTION rows. `SCHEMA.questions` carries all ten of them.
 *
 * SO A RESOURCE ROW WOULD HAVE BEEN A SECOND, EMPTY CARD. The library would list "Practice Paper 1"
 * twice: once built from the questions with all twenty in it, and once from the resources tab with
 * none — same name, nothing inside, no way to tell them apart before opening one. Found by
 * rendering the paper rather than by reading, which is the rule this codebase already has about
 * layout and turns out to hold for data too.
 *
 * THE FACTS GO ON EVERY ROW, not just the first. `allTopics` takes them from the first row it meets
 * for a paper, and that is sheet order — which puts a stem first here. Writing them once would make
 * the paper's name depend on which row somebody happens to drag above another. The note on
 * `allTopics` calls this denormalisation and it is: the cost is ten repeated cells a row, and what
 * it buys is a paper that cannot be renamed by sorting.
 */
function seedPracticePapers() {
  const tQ = read(TAB.questions);
  if (!tQ.sheet) return { error: 'no questions tab — run ensureSchema()' };

  const out = { papers: 0, questions: 0, already: 0 };

  PRACTICE.forEach((p, pi) => {
    /* THE ID IS OURS TO CHOOSE, because nothing joins to it but the question rows themselves. `PP1`
       rather than a generated id so a row typed into the sheet by hand can join the same paper. */
    const paperId = 'PP' + (pi + 1);

    const have = {};
    read(TAB.questions).rows.forEach(r => { const id = S(r.row_id); if (id) have[id] = true; });

    /* WHAT EVERY ROW OF THIS PAPER CARRIES — see the note above. `exam_board` is ours and says so:
       a student who revises from this believing it is Edexcel's and then meets Edexcel's phrasing
       in the hall has been misled by the label rather than taught by the paper. */
    const facts = {
      name: p.name, subject: p.subject, tier: p.tier, key_stage: p.keyStage,
      exam_board: '@family.', resource_type: 'Past paper', year: p.year || '',
    };

    const tag = paperId;
    const write = (rowId, obj) => {
      if (have[rowId]) { out.already++; return; }
      addRow(tQ, Object.assign({ row_id: rowId, paper_id: paperId, active: 'TRUE' }, facts, obj));
      have[rowId] = true;
      out.questions++;
    };

    (p.stems || []).forEach(s => write(tag + '-Q' + s[0] + '-stem', {
      question: s[0], part: '', kind: 'stem', html: s[1], marks: '',
    }));

    p.qs.forEach(q => write(tag + '-Q' + q[0] + (q[1] || ''), {
      question: q[0], part: q[1], kind: 'part',
      /* ---------- `section` IS NOT A TOPIC, AND THE PAGE SAID SO ----------------------------------
         THE TOPIC WENT IN `section` and `paperBody_` draws `<h2>Section {section}</h2>` above the
         first question of each — so the paper opened with "SECTION NUMBER", then "SECTION ALGEBRA",
         which reads as an instruction and is not one. That column is for a paper that genuinely has
         parts, A and B; an Edexcel maths paper does not, and neither does this.

         THE TOPIC IS STILL IN THE LIST because it is what makes the array readable while writing
         it, and it is what a fourth column would hold if one is ever added. Nothing carries it to
         the sheet today: `SCHEMA.questions` has no topic column, and inventing one to hold a label
         nothing reads would be a column to keep in step for nothing. The search already finds a
         question by its words — `paperText_` indexes the stem — so "surds" reaches Q1 without it. */
      marks: q[2], section: '', html: '<p>' + q[4] + '</p>',
      answer: '<p>' + q[5] + '</p>',
      /* EVERY ONE OF THESE IS WORKED OUT ON PAPER AND WRITTEN DOWN, which is `calculation` rather
         than `short` — the distinction the schema draws is about what marking one takes, and a
         four-mark method is not a one-mark recall. */
      answer_type: 'calculation',
      /* ANSWERABLE ON A SCREEN. None of these needs a diagram annotated or a table filled in, which
         is what `needs_print` is for — and saying so is what lets the HTML view offer a box. */
      needs_print: 'FALSE',
    }));
    out.papers++;
  });
